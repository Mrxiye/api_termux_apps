#药物查询api接口https://apis.tianapi.com/yaopin/index?key=ede818a4f86014d25a8027050244ca87&word=青霉素
...
def zhongyao(look , warming):
	...
	conn.request('POST','/zhongyao/index',params,headers)
...
print('\033[0;33m%s\033[0m' % '【搜索药物 by 王熙业】终端应用')
print('\033[0;36m%s\033[0m' % '所有资源来于网络api')
print('\033[0;35m%s\033[0m' % '该文件用于药物查询,需要去apis.tianapi.com网站获取key才可使用，且只有免费100次每天')
print('the requirements of run the codes: pip3 install urllib,json,http.client ')
print('\033[0;32m%s\033[0m' % '=========================================')
os.system('date')
while 1:	
	conn = http.client.HTTPSConnection('apis.tianapi.com')  #接口域名
	look=input('\033[0;32m%s\033[0m' %'(每日100次免费,退出输入：q)输入要搜索的药物名字：')
	if look=='q':
		print('感谢使用！')
		break
	params = urllib.parse.urlencode({'key':'ede818a4f86014d25a8027050244ca87','word':look})
	...
	msg=dict_data['msg']
	code=dict_data['code']
	print('搜药库状态码:' + str(code))
	if msg!='success':
		print('尝试搜索中药.........')
		zhongyao(look , warming)
	else:
		result=dict_data['result']
		list=result['list'][0]
		title=list['title']
